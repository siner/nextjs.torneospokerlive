...
function getDateText(datestring) {
    ...
    let returndate = `${day} de ${month} de ${year} a las ${hour}`

    return returndate
}

/*
const getEventMetaData = (event, venues) => {
    const venue_id = event.properties.Lugar.relation.length
        ? event.properties.Lugar.relation[0].id
        : null

    return {
        id: event.id,
        nombre: event.properties.Nombre.title[0].plain_text,
        venue: venues.find((x) => x.id == venue_id),
        venue_id: venue_id,
        fecha: getDateText(event.properties.Fecha.date.start),
    }
}
...
export const getAllEvents = async () => {
    ...
    return allEvents.map((event) => {
        return getEventMetaData(event, venues)
    })
}

export const getAllEventsOnVenue = async (venue_id) => {
    const events = await notion.databases.query({
        database_id: process.env.EVENTS_DATABASE_ID,
        sorts: [
            {
